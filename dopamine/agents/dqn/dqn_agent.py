...
class DQNAgent(object):
    ...
    def __init__(self,
        ...
        if epsilon_fn not in ['linear', 'adaptive']:
            raise Exception('Unexcepted epsilon function type: %s' %
                            str(epsilon_fn))
        ...
        with tf.device(tf_device):
            ...
            self.state_ph = tf.placeholder(
                tf.uint8, state_shape, name='state_ph')
        ...
        if self.eval_mode:
            ...
            if self.epsilon_fn == 'linear':
                epsilon = linearly_decaying_epsilon(self.epsilon_decay_period, self.training_steps,
                                                    self.min_replay_history, self.epsilon_eval)
            elif self.epsilon_fn == 'adaptive':
                epsilon = adaptive_epsilon(
                    self.sigma, self.num_actions, self.last_td_err, self.epsilon_eval)
            self.epsilon_eval = epsilon
