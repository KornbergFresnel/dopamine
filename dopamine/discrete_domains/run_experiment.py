...
class Runner(object):
    ...
    def _initialize_episode(self):
        ...
        """
        initial_observation = self._environment.reset()
        action, _ = self._agent.begin_episode(initial_observation)
        return action
    ...
    def _run_one_episode(self):
        ...
        while True:
            ...
            if (self._environment.game_over or
                ...
                self._agent.end_episode(reward)
                action, epsilon = self._agent.begin_episode(observation)
            else:
                action, epsilon = self._agent.step(reward, observation)

        self._end_episode(reward)

        return step_number, total_reward, epsilon
    ...
    def _run_one_phase(self, min_steps, statistics, run_mode_str):
        ...
        while step_count < min_steps:
            episode_length, episode_return, epsilon = self._run_one_episode()
            statistics.append({
                '{}_episode_lengths'.format(run_mode_str): episode_length,
                '{}_episode_returns'.format(run_mode_str): episode_return,
                '{}_episode_epsilon'.format(run_mode_str): epsilon,
            })
            ...
            sys.stdout.flush()
        return step_count, sum_returns, num_episodes, sum_epsilon / max(1, step_count)
    ...
    def _run_train_phase(self, statistics):
        ...
        number_steps, sum_returns, num_episodes, ave_epsilon = self._run_one_phase(
            self._training_steps, statistics, 'train')
        average_return = sum_returns / num_episodes if num_episodes > 0 else 0.0
        statistics.append({'train_average_return': average_return, 'train_ave_epsilon': ave_epsilon})
        time_delta = time.time() - start_time
        tf.logging.info('Average undiscounted return per training episode: %.2f',
                        average_return)
        tf.logging.info('Average training steps per second: %.2f',
                        number_steps / time_delta)
        return num_episodes, average_return, ave_epsilon
    ...
    def _run_eval_phase(self, statistics):
        ...
        _, sum_returns, num_episodes, ave_epsilon = self._run_one_phase(
            self._evaluation_steps, statistics, 'eval')
        average_return = sum_returns / num_episodes if num_episodes > 0 else 0.0
        tf.logging.info('Average undiscounted return per evaluation episode: %.2f',
                        average_return)
        statistics.append({'eval_average_return': average_return, 'eval_average_epsilon': ave_epsilon})
        return num_episodes, average_return, ave_epsilon
    ...
    def _run_one_iteration(self, iteration):
        ...
        num_episodes_train, average_reward_train, ave_eps_train = self._run_train_phase(
            statistics)
        num_episodes_eval, average_reward_eval, ave_eps_eval = self._run_eval_phase(
            statistics)
...
class TrainRunner(Runner):
  ...
  def _run_one_iteration(self, iteration):
    ...
    def _run_one_iteration(self, iteration):
        ...
        """
        statistics = iteration_statistics.IterationStatistics()
        num_episodes_train, average_reward_train, ave_eps = self._run_train_phase(
            statistics)

        self._save_tensorboard_summaries(iteration, num_episodes_train,
                                         average_reward_train, ave_eps)
        return statistics.data_lists

    def _save_tensorboard_summaries(self, iteration, num_episodes,
                                    average_reward, ave_eps):
        """Save statistics as tensorboard summaries."""
        summary = tf.Summary(value=[
            tf.Summary.Value(tag='Train/NumEpisodes',
            ...
            tf.Summary.Value(
                tag='Train/AverageEps', simple_value=ave_eps),
        ])
        self._summary_writer.add_summary(summary, iteration)
